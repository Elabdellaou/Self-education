...
<body class="w-100 vh-100">
    ...
    <div class="w-100 h-100 d-flex justify-content-center align-items-center page" id="content"
        ...
        <div class="content w-75 h-75 bg-white position-relative " style="overflow: hidden;">
            <div class="sign-in-sign-up position-absolute">
                <div class="header px-5 pt-3 w-100 d-flex flex-row justify-content-between">
                    ...
                    <div
                        class="checkbox-div fw-bold fs-4 rounded-circle d-flex align-items-center justify-content-center">
                ...
                <div class="sign-in w-100 position-absolute start-50 top-50 translate-middle">
                    <div class="w-50 h-100 d-flex flex-column">
                        <h1 class="mb-4 fw-bold text-start text-nor fs-4 mb-5 title position-relative">Password recovery
                        </h1>
                        <div class="alert alert-danger alert-dismissible fade <?= $errors->any() ? 'show' : 'hide' ?>"
                            role="alert">
                        ...
                        <div class="form-input mb-3 position-relative">
                            <input type="email" class=" w-100 h-100 rounded-3 shadow-primary-input border-nor"
                                id="email_search" name="email_search" placeholder="Email address" autocomplete="off" />
                            <i class="fa-solid fa-envelope position-absolute top-50 translate-middle fs-5 text-nor"
                                style="left: 7%;"></i>
                        </div>
                ...
                <div class="sign-up w-100 position-absolute start-50 top-50 translate-middle">
                    <div class="w-50 h-100 d-flex flex-column">
                        ...
                        <p class="mb-1 fs-6 text-nor w-100 text-center" id="aftersend" style="display:none">Send Email
                        </p>
                ...
                <div class="panel h-75 panel-right position-absolute top-0">
                    <div class="container h-75 d-flex flex-column my-5 align-items-center justify-content-around">
                        <div class="content d-flex flex-column align-items-center">
                            ...
                            <a class="btn btn-outline-light shadow-primary transparent fw-bold" id="sign-up-btn"
                ...
                if (email.val().match(re_validate_email)) {
                    axios.get("/Compte/" + email.val() + "").then(response => {
                        if (response.data.length > 0) {
                            ...
                            result.html(
                                '<div class="d-flex flex-row" id="content_result" style="height:40px;overflow:hidden;"><div class="h-100"><img src="Images/users/' +
                                response.image +
                                '" alt="Image user" style="background-size:cover;width:40px;" class="h-100"></div><div class="h-100 ps-2 pt-2" style="line-height: 2px;font-size:1rem;"><input type="hidden" id="id_user" value="' +
                                response.id + '" name="id_user"><p id="email_user">' + response
                                .email +
                                '</p><p style="color:#6c757d;font-size:0.8rem;" id="name_user" >' +
                                response.name + '</p></div></div>');
                        } else
                            Error_search()
