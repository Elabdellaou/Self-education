<template>
    <div class="w-100 d-flex h-100 flex-column" style="overflow: hidden;">
        <div class="w-100 d-flex flex-column">
            <div class="header pt-0 px-5 w-100 d-flex flex-row justify-content-between">
                <div></div>
            <div id="clock" class="fs-5 d-flex px-3 py-2 flex-row bg-light text-nor">
                <i class="bi bi-alarm-fill pe-2"></i><span>{{ minutes < 10 ? "0" + minutes : minutes }}</span>:<span>{{ secondes < 10 ? "0" + secondes : secondes}}</span>
            </div>
                ...
                <div class="checkbox-div me-4 fw-bold fs-5 rounded-circle" >
                    <label for="#checkbox-mode" class="px-1 py-2 bg-light" >
                        ...
                        <div class="icon text-nor fs-4">
                            <i class="bi bi-brightness-high-fill text-nor"></i>
                        </div>
                    </label>
                </div>
                <div id="close_test" class="px-1 py-2 bg-light text-nor">
                    <span class="text-nor fw-bold fs-4" @click="leftTest()"><i class="fa-solid fa-power-off"></i></span>
                </div>
            </div>
            </div>
        </div>
        <div class="main w-100" style="height:77%;">
            <div class="container h-100">
                <div class="image-svg d-flex align-items-end" style="height:25%;">
                    <img src="/Images/test_1.svg" class="w-100 h-100" alt=""  style="object-fit: contain;">
                </div>
                <div class="question position-relative mb-3">
                    <span class="question-number rounded-pill fs-6 px-3 py-2 text-white">Question Nº {{ (i+1) }}</span>
                    <div class="question-text py-5 px-5 fs-6 rounded-3 text-center">{{ q[i].text }}</div>
                </div>
                <resultVue v-if="q[i].type=='result'" :question="q[i]" v-model="result_text"></resultVue>
                <orderVue v-else-if="q[i].type=='order'" :answers="answers" ></orderVue>
        ...
        <div class="w-100 footer-test">
            <div class="container d-flex flex-row justify-content-between py-3">
                <button class="btn btn-war border-0 fw-bold shadow-primary btn-primary rounded-pill px-4" @click="i>0?i--:i">Back</button>
                <button class="btn btn-war border-0 fw-bold shadow-primary btn-primary rounded-pill px-4" @click="Answer_Question()">Save</button>
            </div>
        </div>
    </div>
</template>
        ...
        if(q.value[i.value].type=='result'){
            if(result_text.value!=''){
                if(i.value<9){
                    ...
                    Answer_DB_End(result_text.value,l)
                result_text.value=''
            ...
            if(selected!=null){
                if(i.value<9){
                    ...
                    Answer_DB_End(selected.textContent,l)
            ...
            if(i.value<9){
                axios.post('/answer_question_order',{
                ...
                let m;
                let s;
                s=secondes.value>=10?secondes.value.toString()+'s':'0'+secondes.value.toString()+'s';
                m=minutes.value%60;
                    m=m>=10?m.toString()+'m ':'0'+m.toString()+'m ';
                h=parseInt(minutes.value/60)
                    h=h>=10?h.toString()+'h ':'0'+h.toString()+'h ';
                let time=h+m+s;
                axios.post('/answer_question_order_end',{
                _token:$('meta[name="csrf-token"]').attr('content'),
                question_id:q.value[i.value].id,
...
function Answer_DB_End(answer,l){
        ...
        h=parseInt(minutes.value/60)
            h=h>=10?h.toString()+'h ':'0'+h.toString()+'h ';
        let time=h+m+s;
        axios.post('/answer_question_end',{
            _token:$('meta[name="csrf-token"]').attr('content'),
            question_id:q.value[i.value].id,
            session_id:store.state.session.id,
            answer_text:answer,
